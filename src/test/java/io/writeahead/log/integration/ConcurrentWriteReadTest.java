...
public class ConcurrentWriteReadTest {
  ...
  @Test
  void testConcurrentWritesAndReadsNoDataLoss() throws IOException, InterruptedException {
    ...
    assertEquals(0, writerExceptions.get(), "Writers should not throw exceptions");
    assertEquals(0, readerExceptions.get(), "Readers should not throw InterruptedException");
    assertTrue(
    ...
    for (LogEntry entry : recovered) {
      ...
      assertFalse(seenEntries.contains(entryData), "Duplicate entry found: " + entryData);
  ...
  @Test
  void testHighThroughputConcurrentWrites() throws IOException, InterruptedException {
    WalConfiguration config = new WalConfiguration.Builder().batchSize(50).logDir(logPath).build();
    WriteAheadLog wal = new WriteAheadLog(config);
  ...
  @Test
  void testStressTestWithSegmentRotation() throws IOException, InterruptedException {
    ...
    assertEquals(
        ...
        recovered.size(),
        String.format(
            "Should recover all %d entries even with segment rotation", totalExpectedEntries));

    // Verify no duplicates
    Set<String> seenHeaders = new HashSet<>();
    for (LogEntry entry : recovered) {
      String header = new String(entry.data(), 0, 10);
      assertFalse(seenHeaders.contains(header), "Duplicate entry detected");
