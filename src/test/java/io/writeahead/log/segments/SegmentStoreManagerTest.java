...
public class SegmentStoreManagerTest {
    ...
    void appendMultipleEntriesInSequence() throws Exception {
        ...
        LogEntry firstLogEntry = new LogEntry(firstPayloadBytes.length, firstPayloadBytes, TIMESTAMP_1000);
        managerUnderTest.append(firstLogEntry);

        byte[] secondPayloadBytes = TEST_ENTRY_DATA_SECOND.getBytes();
        LogEntry secondLogEntry = new LogEntry(secondPayloadBytes.length, secondPayloadBytes, TIMESTAMP_2000);
        managerUnderTest.append(secondLogEntry);

        byte[] thirdPayloadBytes = TEST_ENTRY_DATA_THIRD.getBytes();
        LogEntry thirdLogEntry = new LogEntry(thirdPayloadBytes.length, thirdPayloadBytes, TIMESTAMP_3000);
    ...
    void appendTriggersSegmentRotationWhenSizeExceeded() throws Exception {
        ...
        LogEntry largeLogEntry = new LogEntry(largePayloadBytes.length, largePayloadBytes, TIMESTAMP_1000);
    ...
    void managerHandlesEmptyAppendSafely() throws Exception {
        ...
        LogEntry emptyLogEntry = new LogEntry(emptyPayloadBytes.length, emptyPayloadBytes, TIMESTAMP_1000);
