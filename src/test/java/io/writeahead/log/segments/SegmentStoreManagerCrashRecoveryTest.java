...
public class SegmentStoreManagerCrashRecoveryTest {
    ...
    void setUp() throws Exception {
        ...
        walConfiguration = new WalConfiguration.Builder()
    ...
    void crashBeforeFinalizeDiscardsPendingEntries() throws Exception {
        ...
        LogEntry firstLogEntry = new LogEntry(firstEntryPayloadBytes.length, firstEntryPayloadBytes, TIMESTAMP_1000);
        ...
        LogEntry pendingLogEntry = new LogEntry(pendingEntryPayloadBytes.length, pendingEntryPayloadBytes, TIMESTAMP_2000);
    ...
    void recoveryIsDeterministicAcrossMultipleCrashes() throws Exception {
        for (int crashIndex = 0; crashIndex < 3; crashIndex++) {
            ...
            byte[] entryPayloadBytes = TEST_ENTRY_DATA.getBytes();
            LogEntry logEntry = new LogEntry(entryPayloadBytes.length, entryPayloadBytes, TIMESTAMP_1000 + crashIndex);
    ...
    void validSegmentRecoveredWithUnfinalzedSegmentPresent() throws Exception {
        ...
        LogEntry validLogEntry = new LogEntry(validEntryPayloadBytes.length, validEntryPayloadBytes, TIMESTAMP_1000);
        ...
        LogEntry unfinalzedLogEntry = new LogEntry(unfinalzedEntryPayloadBytes.length, unfinalzedEntryPayloadBytes, TIMESTAMP_2000);
    ...
    void multipleValidSegmentsRecoveredWithUnfinalzedInMiddle() throws Exception {
        ...
        LogEntry firstValidLogEntry = new LogEntry(firstValidEntryPayloadBytes.length, firstValidEntryPayloadBytes, TIMESTAMP_1000);
        ...
        LogEntry secondValidLogEntry = new LogEntry(secondValidEntryPayloadBytes.length, secondValidEntryPayloadBytes, TIMESTAMP_3000);
    ...
    void crashDuringBatchWriteDoesNotCorruptRecovery() throws Exception {
        ...
        for (int entryIndex = 0; entryIndex < 5; entryIndex++) {
            byte[] entryPayloadBytes = TEST_ENTRY_DATA.getBytes();
            LogEntry logEntry = new LogEntry(entryPayloadBytes.length, entryPayloadBytes, TIMESTAMP_1000 + entryIndex);
            firstManagerInstance.append(logEntry);
        }
        ...
        for (int entryIndex = 0; entryIndex < 3; entryIndex++) {
            byte[] entryPayloadBytes = TEST_ENTRY_DATA.getBytes();
            LogEntry logEntry = new LogEntry(entryPayloadBytes.length, entryPayloadBytes, TIMESTAMP_2000 + entryIndex);
