...
public class SegmentEntriesReaderTest {
  ...
  private final SegmentEntriesReader entriesReader =
      new SegmentEntriesReader(new SimpleWalMetrics());

  @Test
  void readSingleEntryFromRegion() throws IOException {
    byte[] serializedRegion =
        createSerializedEntryRegion(
            new LogEntry(HELLO_PAYLOAD_SIZE, HELLO_PAYLOAD.getBytes(), FIRST_ENTRY_TIMESTAMP));

    SegmentEntriesReader.SegmentReadResult readResult =
  ...
  void readMultipleEntriesFromRegion() throws IOException {
    byte[] serializedRegion =
        createSerializedEntryRegion(
            ...
            new LogEntry(
                SMALL_PAYLOAD_SIZE, SECOND_ENTRY_PAYLOAD.getBytes(), SECOND_ENTRY_TIMESTAMP),
            new LogEntry(
                SMALL_PAYLOAD_SIZE, THIRD_ENTRY_PAYLOAD.getBytes(), THIRD_ENTRY_TIMESTAMP));
  ...
  void corruptedEntryCrcThrowsCorruptionException() throws IOException {
    ...
    serializedRegion[serializedRegion.length - 1] =
        (byte) ~serializedRegion[serializedRegion.length - 1];

    CorruptionException caughtException =
        assertThrows(
            CorruptionException.class, () -> entriesReader.readEntriesFromRegion(serializedRegion));
  ...
  void corruptionInMiddleEntryStopsReading() throws IOException {
    ...
    serializedRegion[secondEntryCrcPositionInBuffer + 1] =
        (byte) ~serializedRegion[secondEntryCrcPositionInBuffer + 1];
    serializedRegion[secondEntryCrcPositionInBuffer] =
        (byte) ~serializedRegion[secondEntryCrcPositionInBuffer];
  ...
  void emptyEntryRegionReturnsEmptyList() throws IOException {
    ...
    SegmentEntriesReader.SegmentReadResult readResult =
        entriesReader.readEntriesFromRegion(emptyRegion);
  ...
  private byte[] createSerializedEntryRegion(LogEntry... entriesToSerialize) throws IOException {
    ...
    for (LogEntry entry : entriesToSerialize) {
      ...
      byte[] serializedEntryWithCrc =
          EntrySerdes.serializeEntryWithCrc(
              entry.timestamp(), entry.size(), entry.data(), entryCrc);
