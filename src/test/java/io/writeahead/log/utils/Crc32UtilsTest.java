...
public class Crc32UtilsTest {
  ...
  void computeEntryCrcWithSimplePayloadProducesNonZeroPositiveCrc() throws IOException {
    ...
    long computedCrc =
        Crc32Utils.computeEntryCrc(ARBITRARY_TIMESTAMP, SMALL_PAYLOAD_SIZE, simplePayloadBytes);
  ...
  void computeEntryCrcIdenticalInputProducesIdenticalCrcMultipleTimes() throws IOException {
    ...
    long crcFirstComputation =
        Crc32Utils.computeEntryCrc(ARBITRARY_TIMESTAMP, SMALL_PAYLOAD_SIZE, simplePayloadBytes);
    long crcSecondComputation =
        Crc32Utils.computeEntryCrc(ARBITRARY_TIMESTAMP, SMALL_PAYLOAD_SIZE, simplePayloadBytes);
    long crcThirdComputation =
        Crc32Utils.computeEntryCrc(ARBITRARY_TIMESTAMP, SMALL_PAYLOAD_SIZE, simplePayloadBytes);
  ...
  void computeEntryCrcDetectsDifferentTimestamps() throws IOException {
    ...
    long crcTimestamp1000 =
        Crc32Utils.computeEntryCrc(1000L, SMALL_PAYLOAD_SIZE, simplePayloadBytes);
    long crcTimestamp1001 =
        Crc32Utils.computeEntryCrc(1001L, SMALL_PAYLOAD_SIZE, simplePayloadBytes);
    long crcTimestamp2000 =
        Crc32Utils.computeEntryCrc(2000L, SMALL_PAYLOAD_SIZE, simplePayloadBytes);
  ...
  void computeEntryCrcDetectsDifferentPayloads() throws IOException {
    ...
    long crcSimplePayload =
        Crc32Utils.computeEntryCrc(timestamp, SMALL_PAYLOAD_SIZE, SIMPLE_PAYLOAD.getBytes());
    long crcAlternativePayload =
        Crc32Utils.computeEntryCrc(timestamp, SMALL_PAYLOAD_SIZE, ALTERNATIVE_PAYLOAD.getBytes());
  ...
  void computeEntryCrcEmptyPayloadProducesConsistentCrc() throws IOException {
    ...
    long crcFirstComputation =
        Crc32Utils.computeEntryCrc(ARBITRARY_TIMESTAMP, 0, emptyPayloadBytes);
    long crcSecondComputation =
        Crc32Utils.computeEntryCrc(ARBITRARY_TIMESTAMP, 0, emptyPayloadBytes);
  ...
  void computeEntryCrcLargePayloadProducesValidCrc() throws IOException {
    ...
    long computedCrc =
        Crc32Utils.computeEntryCrc(ARBITRARY_TIMESTAMP, LARGE_PAYLOAD_SIZE, largePayloadBytes);
  ...
  void computeEntryCrcExtremeTimestampsProducesMostlyUniqueCrcs() throws IOException {
    ...
    for (int index = 0; index < extremeTimestampValues.length; index++) {
      computedCrcsForExtremeTimestamps[index] =
          Crc32Utils.computeEntryCrc(
              extremeTimestampValues[index], SMALL_PAYLOAD_SIZE, simplePayloadBytes);
    }
  ...
  void computeEntryCrcMatchesComputeOfSerializedEntry() throws IOException {
    ...
    byte[] serializedEntryBytes =
        EntrySerdes.serializeEntrySanseCrc(timestamp, size, simplePayloadBytes);
