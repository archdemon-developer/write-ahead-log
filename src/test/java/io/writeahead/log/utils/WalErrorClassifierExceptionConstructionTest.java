...
public class WalErrorClassifierExceptionConstructionTest {
  ...
  void classifyRecoveryErrorWithSegmentTooSmall() {
    RecoveryException result =
        WalErrorClassifier.classifyRecoveryError(
            RecoveryType.SEGMENT_TOO_SMALL, "Segment file is smaller than minimum threshold");
  ...
  void classifyRecoveryErrorWithPartialEntryAtEof() {
    RecoveryException result =
        WalErrorClassifier.classifyRecoveryError(
            RecoveryType.PARTIAL_ENTRY_AT_EOF, "Incomplete entry found at end of file");
  ...
  void classifyRecoveryErrorWithMissingSegmentFile() {
    RecoveryException result =
        WalErrorClassifier.classifyRecoveryError(
            RecoveryType.MISSING_SEGMENT_FILE, "Expected segment file not found");
  ...
  void classifyRecoveryErrorWithUnreadableSegment() {
    RecoveryException result =
        WalErrorClassifier.classifyRecoveryError(
            RecoveryType.UNREADABLE_SEGMENT, "Cannot read segment file");
  ...
  void classifyRecoveryErrorWithIncompleteSegment() {
    RecoveryException result =
        WalErrorClassifier.classifyRecoveryError(
            RecoveryType.INCOMPLETE_SEGMENT, "Segment header written but not finalized");
  ...
  void classifyConcurrencyErrorWithLockTimeout() {
    ConcurrencyException result =
        WalErrorClassifier.classifyConcurrencyError(
            ConcurrencyErrorType.LOCK_TIMEOUT, "Failed to acquire write lock within timeout");
  ...
  void classifyConcurrencyErrorWithInterrupted() {
    ConcurrencyException result =
        WalErrorClassifier.classifyConcurrencyError(
            ConcurrencyErrorType.INTERRUPTED, "Thread interrupted during lock acquisition");
  ...
  void classifyConcurrencyErrorWithDeadlock() {
    ConcurrencyException result =
        WalErrorClassifier.classifyConcurrencyError(
            ConcurrencyErrorType.DEADLOCK, "Potential deadlock detected between writers");
  ...
  void classifyConcurrencyErrorWithUnknown() {
    ConcurrencyException result =
        WalErrorClassifier.classifyConcurrencyError(
            ConcurrencyErrorType.UNKNOWN, "Unknown concurrency error occurred");
