...
public class SegmentFooterTest {
  ...
  void createFooterProducesValidFooterWithCorrectFields() throws Exception {
    SegmentFooter createdFooter =
        SegmentFooter.create(REFERENCE_ENTRY_COUNT, MINIMUM_TIMESTAMP, MAXIMUM_TIMESTAMP);
  ...
  void serializationAndDeserializationPreservesAllFooterFields() throws Exception {
    SegmentFooter originalFooter =
        SegmentFooter.create(
            REFERENCE_ENTRY_COUNT, SMALL_MINIMUM_TIMESTAMP, SMALL_MAXIMUM_TIMESTAMP);
  ...
  void corruptedCompleteMarkerRendersFooterInvalid() throws Exception {
    SegmentFooter validFooter =
        SegmentFooter.create(SMALL_ENTRY_COUNT, MINIMUM_TIMESTAMP, MAXIMUM_TIMESTAMP);
  ...
  void multipleEntryCountsSerializeAndDeserializeCorrectly() throws Exception {
    for (int entryCount = 0; entryCount <= 10; entryCount++) {
      SegmentFooter footerForCount =
          SegmentFooter.create(entryCount, MINIMUM_TIMESTAMP, MAXIMUM_TIMESTAMP);
  ...
  void largeEntryCountPreservedThroughSerializationRoundTrip() throws Exception {
    SegmentFooter footerWithLargeCount =
        SegmentFooter.create(LARGE_ENTRY_COUNT, MINIMUM_TIMESTAMP, MAXIMUM_TIMESTAMP);
  ...
  void zeroEntryCountIsValidAndPreserved() throws Exception {
    SegmentFooter footerWithZeroCount =
        SegmentFooter.create(ZERO_ENTRY_COUNT, MINIMUM_TIMESTAMP, MAXIMUM_TIMESTAMP);
  ...
  void multipleDistinctFootersSerializeIndependently() throws Exception {
    ...
    assertTrue(
        firstFooterDeserialized.isValid()
            && secondFooterDeserialized.isValid()
            && thirdFooterDeserialized.isValid());
  }
  ...
  void minMaxTimestampsPreservedAccuratelyThroughRoundTrip() throws Exception {
    ...
    SegmentFooter footerWithExtremeTimestamps =
        SegmentFooter.create(SMALL_ENTRY_COUNT, minValue, maxValue);
