...
public class WalConfigurationTest {
  ...
  void retryBackoffMultiplierOneIsValid() {
    ...
    assertEquals(
        VALID_RETRY_BACKOFF_MULTIPLIER_NO_EXPONENTIAL, config.retryBackoffMultiplier(), 0.01);
  }
  ...
  void logDirNullThrowsIllegalArgumentException() {
    assertThrows(
        IllegalArgumentException.class, () -> new WalConfiguration.Builder().logDir(null).build());
  }
