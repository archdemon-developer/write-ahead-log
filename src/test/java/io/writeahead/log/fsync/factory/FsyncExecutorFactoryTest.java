...
public class FsyncExecutorFactoryTest {
  ...
  void createWithEveryBatchStrategyReturnsEveryBatchFsyncExecutor() {
    FsyncRetryStrategy fsyncRetryStrategy =
        new ExponentialBackoffRetryStrategy(3, 10, 1.0, new SimpleWalMetrics());

    FsyncExecutor createdFsyncExecutor =
        FsyncExecutorFactory.create(
            FsyncStrategy.FSYNC_EVERY_BATCH, fsyncRetryStrategy, fileStream);
  ...
  void createWithEveryEntryStrategyReturnsEveryEntryFsyncExecutor() {
    ...
    FsyncExecutor createdFsyncExecutor =
        FsyncExecutorFactory.create(
            FsyncStrategy.FSYNC_EVERY_ENTRY, fsyncRetryStrategy, fileStream);
  ...
  void createMultipleCallsSameStrategyReturnsDifferentInstances() {
    ...
    FsyncExecutor firstCreatedFsyncExecutor =
        FsyncExecutorFactory.create(
            FsyncStrategy.FSYNC_EVERY_BATCH, fsyncRetryStrategy, fileStream);

    FsyncExecutor secondCreatedFsyncExecutor =
        FsyncExecutorFactory.create(
            FsyncStrategy.FSYNC_EVERY_BATCH, fsyncRetryStrategy, fileStream);
  ...
  void createAllStrategiesProduceSomeExecutor() {
    ...
    for (FsyncStrategy fsyncStrategy : FsyncStrategy.values()) {
      FsyncExecutor createdFsyncExecutor =
          FsyncExecutorFactory.create(fsyncStrategy, fsyncRetryStrategy, fileStream);
