...
public class WalErrorClassifier {
  ...
  public static WalException classifyIOException(IOException ex, String operation) {
    String message = ex.getMessage() != null ? ex.getMessage() : "";

    switch (ex) {
      case AccessDeniedException accessDeniedException -> {
        return new PermanentIOException(
            "Permission denied during " + operation, ErrorContext.PERMISSION_DENIED, operation);
      }
      case FileNotFoundException fileNotFoundException -> {
        return new PermanentIOException(
            "File not found during " + operation, ErrorContext.FILE_NOT_FOUND, operation);
      }
    ...
    for (Map.Entry<String, ErrorContext> pattern : ERROR_PATTERNS.entrySet()) {
      if (message.contains(pattern.getKey())) {
        ...
        if (isTransientContext(context)) {
          return new TransientIOException(message, context, operation);
        } else {
          return new PermanentIOException(message, context, operation);
        }
      }
    }

    return new PermanentIOException(
        "I/O error during " + operation + ": " + message, ErrorContext.UNKNOWN_IO_ERROR, operation);
  }
  ...
  public static CorruptionException classifyCorruption(
    ...
    return new CorruptionException(
        "Corruption detected in segment "
            + segmentName
            + " at offset "
            + byteOffset
            + ": "
  ...
  public static RecoveryException classifyRecoveryError(RecoveryType type, String details) {

    return new RecoveryException("Recovery error: " + details, type);
  }

  public static ConcurrencyException classifyConcurrencyError(
      ConcurrencyErrorType type, String details) {

    return new ConcurrencyException("Concurrency error: " + details, type);
  }

  private static boolean isTransientContext(ErrorContext context) {
    return context == ErrorContext.RESOURCE_BUSY || context == ErrorContext.NO_MEMORY;
  }
}
